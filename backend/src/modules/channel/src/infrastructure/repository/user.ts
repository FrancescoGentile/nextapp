  ...
  public static async create(driver: Driver): Promise<Neo4jUserRepository> {
    ...
    try {
      await session.writeTransaction((tx) =>
        tx.run(
          `CREATE CONSTRAINT CHANNEL_unique_user_id IF NOT EXISTS
           FOR (u:CHANNEL_User)
  ...
  public async get_user_role(user_id: UserID): Promise<UserRole | null> {
    ...
    try {
      const res = await session.readTransaction((tx) =>
        tx.run(`MATCH (u:CHANNEL_User { id: $id }) RETURN u.admin as admin`, {
  ...
  public async set_user_role(
    ...
    try {
      const res = await session.writeTransaction((tx) =>
        tx.run(`MATCH (u:CHANNEL_User { id: $id }) SET u.admin = $admin`, {
          id: user_id.to_string(),
