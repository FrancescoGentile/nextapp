...
export class NextChannelInfoService implements ChannelInfoService {
  ...
  public async update_channel(
    ...
    const is_pres = await this.channel_repo.is_president(
      requester,
      channel.id!
    );
    if (!is_pres) {
  ...
  public async get_pres_channels(
    ...
    const channels = await this.channel_repo.get_pres_channels(
      requester,
      options
    );
    if (channels === null) {
